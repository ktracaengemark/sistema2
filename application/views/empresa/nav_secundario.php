	...
	<ul class="nav nav-sidebar">
		<li <?php if (preg_match("/prontuario\b/", $_SERVER['REQUEST_URI'])) echo 'class=active'; //(.)+\/prontuario/   ?>>
			<a href="<?php echo base_url() . 'empresa/prontuario/' . $_SESSION['Empresa']['idApp_Empresa']; ?>">
				<span class="glyphicon glyphicon-user"> </span> Dados da Empresa & Prest.<span class="sr-only">(current)</span>
			</a>
		</li>
	...
	<ul class="nav nav-sidebar">
		<li>
			<div class="text-center t">
				<h3><?php echo '<strong>' . $_SESSION['Empresa']['NomeEmpresa'] . '</strong><br><small>Identificador: ' . $_SESSION['Empresa']['idApp_Empresa'] . '</small>' ?></h3>
			</div>
		</li>
	</ul>
</div>
